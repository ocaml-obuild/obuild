...
type use_thread_flag = Prepare_types.use_thread_flag = NoThread | WithThread
type thread_type = Prepare_types.thread_type = VMThread | PosixThread | DefaultThread | NoThreads
type ocaml_file_type = Prepare_types.ocaml_file_type = GeneratedModule | SimpleModule
...
(** Helper: Resolve PPX flags for a specific module
    ...
    This complex logic handles PPX dependencies and options, ensuring
    only one PPX is used per module and that ppxopt arguments match.
...
let analyze_module_dependencies srcFile hier pp file_search_paths =
  ...
  let cwdDeps = List.map (fun x ->
      maybe (Hier.make [x]) (fun z -> Hier.append z x) (Hier.parent hier)
    ) cwdDepsInDir in
...
(** Helper: Discover modules within a directory
    ...
    When a module is represented as a directory, this function scans
    the directory to find all sub-modules (both files and subdirectories).
...
let discover_directory_modules srcDir hier =
  let modules = Filesystem.list_dir_pred_map (fun f ->
      ...
      then
        (* Avoid directories like .git/.svn etc. *)
...
(** Helper: Compute all dependency paths for compilation and linking
    ...
    Calculates include paths and linking paths for both OCaml and C compilation,
    handling internal and system dependencies, and different compilation modes
    (normal, debug, profiling).

    @param bstate build state
    @param target current target
    @return tuple of (depIncludePaths, depIncludePathsD, depIncludePathsP,
                      depLinkingPaths, cdepsIncludePaths, cCamlIncludePath)
 *)
let compute_dependency_paths bstate target =
  ...
  let cdepsIncludePaths : filepath list =
    ...
    @ List.concat (List.map (fun (cpkg,_) ->
        (Hashtbl.find bstate.bstate_config.project_cpkgs cpkg).cpkg_conf_includes) cbits.target_cpkgs)
  in
  let cCamlIncludePath = fp (Analyze.get_ocaml_config_key "standard_library" bstate.bstate_config) in

  (depIncludePaths, depIncludePathsD, depIncludePathsP, depLinkingPaths, cdepsIncludePaths, cCamlIncludePath)

(** Helper: Get the compile step for a module descriptor

    Determines whether a module needs interface compilation, module compilation,
    or directory packing based on its descriptor.
 *)
...
(** Helper: Build the module compilation steps DAG
    ...
    Processes module dependencies in topological order and builds the steps DAG.
    Uses a work queue to process modules with no dependencies first, then removes
    them and processes their dependents.
 *)
...
(** Helper: Add C compilation tasks to the DAGs
    ...
    Processes C source files, determines their header dependencies,
    and adds compilation tasks to both the steps DAG and files DAG.
 *)
...
let get_modules_desc bstate target toplevelModules =
  ...
  let get_one hier =
    ...
    let module_desc_ty =
      ...
      then discover_directory_modules srcDir hier
      else (
        ...
        let pp = match target.target_obits.target_pp,per_pp with
          ...
          | None,Some preprocessor | Some _, Some preprocessor ->
            ...
            Pp.append perPP (Ppx_resolver.get_syntax_pp bstate preprocessor (List.map fst extraDeps))
          | Some preprocessor,None ->
            ...
            *)
            let extraDeps = List.concat (List.map (fun x -> x.target_extra_builddeps) per_settings) in
            Pp.append targetPP (Ppx_resolver.get_syntax_pp bstate preprocessor (List.map fst extraDeps))
        in
        ...
        let (cwdDeps, otherDeps, use_thread) = analyze_module_dependencies srcFile hier pp file_search_paths in
...
let prepare_target_ bstate buildDir target toplevelModules =
  ...
  let (depIncludePaths, depIncludePathsD, depIncludePathsP, depLinkingPaths, cdepsIncludePaths, cCamlIncludePath) =
    compute_dependency_paths bstate target in
