(** Project configuration file (.obuild) types and utilities
    ...
    This module defines the types for representing obuild project files.
    Parsing is handled by obuild_parser and obuild_validate modules.
    Use Project_read.read() to parse project files.
*)
...
(** Helper: Validate that modules exist in target source directories
    Skips modules that will be auto-generated by cstubs *)
let check_modules_exists target modules =
  ...
  let cstubs_generated_modules = match target.target_cstubs with
    | Some cstubs ->
      let foreign_name = String.capitalize_ascii (cstubs.cstubs_external_library_name ^ "_generated") in
      let types_name = String.capitalize_ascii cstubs.cstubs_generated_types in
      let entry_name = String.capitalize_ascii cstubs.cstubs_generated_entry_point in
