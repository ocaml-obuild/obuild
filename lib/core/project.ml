...
(** Register output modules from generators so they can be found during validation.
    Scans source directories for files matching generator suffixes and registers
    the output modules in Hier. *)
