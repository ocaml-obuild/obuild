...
type 'a optionHandling =
  | Handled of 'a
  | NotHandled

let raise_if_strict strict s =
  if strict then
    raise (InvalidConfFile s)
  else
    Printf.eprintf "config warning: %s\n" s
...
let rec get_block (lvl : int) (lines : (int * string) list) :
...
let do_block2 _ mempty parseM accu args cont = accu (process_chunk parseM (mempty args) cont)
...
let parse_stdlib value =
  match string_lowercase value with
  | "none" | "no" -> Stdlib_None
...
let parse_runtime_bool _ = function
  ...
  | flag ->
      if string_startswith "$" flag then BoolVariable (string_drop 1 flag) else BoolVariable flag

let parse_module_name value =
  let wrap_module_nice s = Hier.make [ Modname.wrap (string_capitalize s) ] in
  List.map wrap_module_nice (Utils.parseCSV value)

let parse_per strict (acc : target_extra) line cont =
  match Utils.toKV line with
  | _, None ->
      raise_if_strict strict "no block in per";
      acc
  | k, Some v -> (
      let (value : string) = String.concat "\n" (v :: List.map snd cont) in
      match string_lowercase k with
      | "builddepends" | "builddeps" | "build-deps" ->
          {
            acc with
            target_extra_builddeps = parse_deps Libname.of_string value @ acc.target_extra_builddeps;
          }
      | "oflags" ->
          { acc with target_extra_oflags = acc.target_extra_oflags @ string_words_noempty value }
      | "pp" -> { acc with target_extra_pp = Some (Pp.Type.of_string value) }
      | _ ->
          raise_if_strict strict ("unexpected item in : " ^ k);
          acc)

let parse_otarget t k value =
  match k with
  | "builddepends" | "builddeps" | "build-deps" ->
      Handled { t with target_builddeps = parse_deps Libname.of_string value @ t.target_builddeps }
  | "path" | "srcdir" | "src-dir" ->
      Handled { t with target_srcdir = List.map (fun p -> fp p) (Utils.parseCSV value) }
  | "preprocessor" | "pp" -> Handled { t with target_pp = Some (Pp.Type.of_string value) }
...
let parse_ctarget t k value =
  match k with
  | "cdir" | "c-dir" -> Handled { t with target_cdir = fp value }
  | "csources" | "c-sources" ->
      Handled { t with target_csources = t.target_csources @ parse_filenames value }
  | "cflags" | "c-flags" | "ccopts" | "ccopt" | "c-opts" ->
      Handled { t with target_cflags = t.target_cflags @ string_words_noempty value }
  | "c-libpaths" ->
      Handled
        { t with target_clibpaths = t.target_clibpaths @ List.map fp (string_words_noempty value) }
...
module Library = struct
  ...
  let rec parse strict acc line cont =
    ...
    | k, None -> (
        ...
        | [] ->
            raise_if_strict strict "unknown empty block in library";
            acc
        | blockName :: args -> (
            match string_lowercase blockName with
            | "sub" | "sublib" | "library" ->
                let doSub =
                  do_block "library" (make_prefix acc.name) (parse strict) (fun obj ->
                      { acc with subs = obj :: acc.subs })
                in
                doSub args cont
            | "per" ->
                let t = acc.target in
                ...
                doPer args cont
            | _ ->
                raise_if_strict strict ("unexpected block name in library: " ^ blockName);
                acc))
    | k, Some v -> (
        let (value : string) = String.concat "\n" (v :: List.map snd cont) in
        match string_lowercase k with
...
module Executable = struct
  ...
  let parse_common strict sectionName setMain setTarget myTarget other acc line cont =
    ...
    | k, None -> (
        match string_words_noempty k with
        | [] ->
            raise_if_strict strict ("unknown empty block in " ^ sectionName ^ " " ^ k);
            acc
        | blockName :: args -> (
            match string_lowercase blockName with
            | "per" ->
    ...
    | k, Some v -> (
        let (value : string) = String.concat "\n" (v :: List.map snd cont) in
        match string_lowercase k with
        | "main" | "mainis" | "main-is" -> setMain acc (fn value)
        | k -> (
            try
              let f = List.assoc k other in
              f acc value
            with Not_found -> setTarget acc (parse_target strict myTarget k value)))

  let parse strict obj =
    parse_common strict "executable"
      (fun acc main -> { acc with main })
      (fun acc target -> { acc with target })
      obj.target [] obj
...
module Test = struct
  ...
  let parse strict obj =
    Executable.parse_common strict "test"
      (fun acc main -> { acc with main })
      (fun acc target -> { acc with target })
      obj.target
      [
...
module Bench = struct
  type t = {
    name : string;
    main : filename;
    target : target; (* TODO add bench type *)
  }
...
module Example = struct
  ...
  let parse strict obj =
    Executable.parse_common strict "example"
      (fun acc main -> { acc with main })
      (fun acc target -> { acc with target })
      obj.target [] obj
...
module Flag = struct
  ...
  let make args = { name = get_context_name "flag" args; description = ""; default = None }

  let parse strict acc line cont =
    match Utils.toKV line with
    | k, None ->
        raise_if_strict strict ("unexpected item in flag " ^ k);
        acc
    | k, Some v -> (
        let (value : string) = String.concat "\n" (v :: List.map snd cont) in
        match string_lowercase k with
  ...
  let find flags name =
    try Some (List.find (fun fl -> fl.name = name) flags) with Not_found -> None
end
...
let findPath () =
  ...
  match
    List.find_all
      (fun ent -> (not (string_startswith "." ent)) && string_endswith ".obuild" ent)
      ents
  with
  | [] -> raise NoConfFile
  | [ x ] -> fp x
...
let parse strict lines =
  let parse_library acc =
    do_block "library" Library.make_from_string (Library.parse strict) (fun obj ->
        { acc with libs = obj :: acc.libs })
  in
  let parse_executable acc =
    do_block "executable" Executable.make (Executable.parse strict) (fun obj ->
        { acc with exes = obj :: acc.exes })
  in
  let parse_test acc =
    do_block "test" Test.make (Test.parse strict) (fun obj -> { acc with tests = obj :: acc.tests })
  in
  let parse_example acc =
    do_block "example" Example.make (Example.parse strict) (fun obj ->
        { acc with examples = obj :: acc.examples })
  in
  ...
  let parse_root acc (line : string) (cont : (int * string) list) =
    ...
    | k, None -> (
        match string_words_noempty k with
        | [] ->
            raise_if_strict strict "unknown empty block";
            acc
        | blockName :: args -> (
            match string_lowercase blockName with
            ...
            | "bench" ->
                raise_if_strict strict ("unimplemented section: " ^ blockName);
                acc
            | "example" -> parse_example acc args cont
            | _ ->
                raise_if_strict strict ("unknown block name: " ^ blockName);
                acc))
    | k, Some v -> (
        let (value : string) = String.concat "\n" (v :: List.map snd cont) in
        match string_lowercase k with
        ...
        | "description" -> { acc with description = value }
        | "license" | "licence" -> { acc with license = value }
        | "license-file" | "licence-file" -> { acc with license_file = Some (fp value) }
        | "homepage" -> { acc with homepage = value }
        | "tools" ->
            { acc with extra_tools = List.map fn (string_words_noempty value) @ acc.extra_tools }
        | "authors" -> { acc with authors = Utils.parseCSV value }
        | "author" -> { acc with authors = [ value ] }
        | "extra-srcs" ->
            { acc with extra_srcs = List.map fp (Utils.parseCSV value) @ acc.extra_srcs }
        | "obuild-ver" -> { acc with obuild_ver = user_int_of_string "obuild-ver" value }
        | "ocamlversion" | "ocaml-version" ->
            { acc with ocaml_ver = Expr.parse "ocaml-version" value }
        ...
        | k ->
            raise_if_strict strict ("unknown key: " ^ k);
            acc)
  in
