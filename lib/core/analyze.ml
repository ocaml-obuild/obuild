...
let get_ocaml_config_key_global key = get_ocaml_config_key_hashtbl key (Prog.get_ocaml_config ())
...
let get_pkg_deps target project =
  let pkgs =
    Taskdep.linearize project.project_pkgdeps_dag Taskdep.FromParent [ Target target.target_name ]
  in
  List.rev
    (list_filter_map
       (fun pkg ->
...
let prepare projFile user_flags =
  ...
  let () =
    ...
    if
      not
        (List.exists
           (fun p ->
             String_utils.startswith (fp_to_string p) (fp_to_string stdlibPath)
             || String_utils.startswith (fp_to_string stdlibPath) (fp_to_string p))
  ...
  let internalLibs =
    List.map (fun lib -> lib.Project.Library.name.Libname.main_name) projFile.Project.libs
  in
  ...
  if gconf.dump_dot then (
    ...
    let dotIContent = Dag.to_dot Name.to_string "internal-dependencies" true targetsDag in
