  ...
  let lexer s =
    ...
    let eat_symbol o =
      let tok, no =
        ...
        try
          let tok = process_one_char s.[o] next in
          (tok, o + 1)
        with Not_found ->
    ...
    let rec loop o =
      if o >= len then
        []
      else begin (* TODO skip chunk of space in one go *)
        if
          s.[o] == ' ' || s.[o] == '\t'
        then
          loop (o + 1)
        else if is_symbol_char s.[o] then
          let sym, no = eat_symbol o in
          sym :: loop no
        else if (s.[o] >= 'a' && s.[o] <= 'z') || (s.[o] >= 'A' && s.[o] <= 'Z') then
          let id, no = eat_ident o in
          ID id :: loop no
        else if is_version_char s.[o] then
          let ver, no = eat_version o in
          VER ver :: loop no
        else
...
let compare_version v1 v2 =
  let skip i p s e =
    let rec loop i = if i = e then i else if p s.[i] then loop (i + 1) else i in
    loop i
  in
  let split_version v =
    let p1, rest =
      match String_utils.split ':' v ~limit:2 with
      | [ _ ] -> ("", v)
      | [ p1; rest ] -> (p1, rest)
    in
    let p1, p2, p3 =
      match String_utils.split '-' rest ~limit:2 with
      | [ _ ] -> (p1, rest, "")
      | [ p2; p3 ] -> (p1, p2, p3)
    in
    (p1, p2, p3)
  ...
  let compare_part p1 p2 =
    ...
    let rec loop i1 i2 =
      ...
      | true, true -> 0
      | true, false ->
          let end2 = skip i2 (fun c -> c = '0') p2 l2 in
          if end2 = l2 then 0 else -1
      | false, true ->
          let end1 = skip i1 (fun c -> c = '0') p1 l1 in
          if end1 = l1 then 0 else 1
      | false, false -> (
          match (is_digit p1.[i1], is_digit p2.[i2]) with
          | true, true ->
              compare_numbers (skip i1 (fun c -> c = '0') p1 l1) (skip i2 (fun c -> c = '0') p2 l2)
          | true, false -> -1
          | false, true -> 1
          | false, false ->
              let comp = Char.compare p1.[i1] p2.[i2] in
