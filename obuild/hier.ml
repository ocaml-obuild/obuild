...
type hier = { _hier : Modname.t list }
...
let hier_to_string x = String.concat "." (List.map Modname.to_string x._hier)
let hier_of_string x =
  let l = string_split '.' x in
  hier (List.map Modname.of_string l)
...
let hier_to_dirpath x =
  if List.length x._hier > 1 then
    fp (String.concat Filename.dir_sep (List.map Modname.to_dir $ list_init x._hier))
  else
    currentDir
...
let add_prefix prefix_path hier =
  if List.length hier._hier <= 1 then 
    ...
    let to_fp =
      fp (String.concat Filename.dir_sep (List.map Modname.to_dir $ list_init hier._hier)) in
...
let get_filename path hier ext = 
  if Hashtbl.mem hiers hier then Hashtbl.find hiers hier
    ...
    let modname = Modname.to_string (hier_leaf hier) in
