...
let configure argv =
  ...
  let user_set_flags s =
    let tweak =
      if string_startswith "-" s then Configure.ClearFlag (string_drop 1 s) else Configure.SetFlag s
    in
    user_flags := tweak :: !user_flags
  in
  let set_target_options field value () =
    let opt_name =
      if List.mem field [ "examples"; "benchs"; "tests" ] then "build-" ^ field else field
    in
  ...
  let opts =
    [
      ...
      ( "--executable-as-obj",
        Arg.Unit (set_target_options "executable-as-obj" true),
        "output executable as obj file" );
...
let mainBuild argv =
  ...
  let build_options =
    [
      ...
      ( "--dot",
        Arg.Unit (fun () -> gconf.dump_dot <- true),
        "dump dependencies dot files during build" );
      ( "--noocamlmklib",
        Arg.Unit (fun () -> gconf.ocamlmklib <- false),
        "do not use ocamlmklib when linking C code" );
    ]
  in
...
let mainInfer argv =
  let anon = ref [] in
  Arg.parse_argv (Array.of_list argv) [] (fun s -> anon := s :: !anon) (usageStr "infer");
  ...
  Arg.parse_argv (Array.of_list argv)
    [
      ( "--destdir",
        ...
        "override destination where to install (default coming from findlib configuration)" );
      ( "--opam",
        Arg.Set opam_install,
        "only create the .install file for opam (do not copy the files)" );
    ]
    (fun s -> failwith ("unknown option: " ^ s))
    (usageStr "install");
    ...
    let results =
      List.map
        (fun test ->
          ...
          let outputName =
            Utils.to_exe_name Normal Native (Target.get_target_dest_name testTarget)
          in
...
let parseGlobalArgs () =
  ...
  in
  let printHelp () =
    printf "a rescue team has been dispatched\n";
    exit 0
  in
    ...
    | x :: xs ->
        if String.length x > 0 && x.[0] = '-' then
          let retXs =
            ...
            | "-v" | "--verbose" ->
                gconf.verbosity <- Verbose;
                xs
            | "--color" ->
                gconf.color <- true;
                xs
            | "-vv" | "--debug" ->
                gconf.verbosity <- Debug;
                xs
            | "-vvv" | "--debug+" | "--debug-with-cmd" ->
                gconf.verbosity <- DebugPlus;
                xs
            | "-q" (* for quiet *) | "--silent" ->
                gconf.verbosity <- Silent;
                xs
            | "--strict" ->
                gconf.strict <- true;
                xs
