...
let cmd_configure =
  let cmd = Cli.command "configure"
    ~doc:"Prepare to build the package"
    ~description:"Configures the build system and detects dependencies. Sets up compilation flags and options."
    ~run:(fun ctx ->
      ...
      List.iter (fun s ->
        let tweak =
          if string_startswith "-" s then Configure.ClearFlag (string_drop 1 s)
          else Configure.SetFlag s
        in
...
let cmd_build =
  let cmd = Cli.command "build"
    ~doc:"Make this package ready for installation"
  ...
  cmd
  |> Cli.help_flag
  |> Cli.option_int "jobs" ~short:'j' ~placeholder:"N"
      ~default:gconf.parallel_jobs ~doc:"Maximum number of jobs in parallel"
...
let cmd_clean =
  let cmd = Cli.command "clean"
    ~doc:"Clean up after a build"
...
let cmd_sdist =
  let cmd = Cli.command "sdist"
    ~doc:"Generate a source distribution file (.tar.gz)"
  ...
  cmd
  |> Cli.help_flag
  |> Cli.flag "snapshot" ~doc:"Build a snapshot of the project"

(* ===== Install Command ===== *)

let cmd_install =
  let cmd = Cli.command "install"
    ~doc:"Install this package"
  ...
  cmd
  |> Cli.help_flag
  |> Cli.option_string "destdir" ~placeholder:"DIR"
      ~doc:"Override destination where to install"
  |> Cli.flag "opam" ~doc:"Only create the .install file for opam"

(* ===== Test Command ===== *)

let cmd_test =
  let cmd = Cli.command "test"
    ~doc:"Run the tests"
  ...
  cmd
  |> Cli.help_flag
  |> Cli.flag "output" ~doc:"Show test outputs"

(* ===== Init Command ===== *)

let cmd_init =
  let cmd = Cli.command "init"
...
let cmd_get =
  let cmd = Cli.command "get"
    ~doc:"Get project metadata field"
...
let cmd_infer =
  let cmd = Cli.command "infer"
    ~doc:"Infer module dependencies (unimplemented)"
  ...
  cmd
  |> Cli.help_flag
  |> Cli.positionals "modules" ~placeholder:"MODULE..." ~doc:"Modules to infer"
...
let cmd_generate_completion =
  let cmd = Cli.command "completion"
    ~doc:"Generate shell completion scripts"
    ~description:"Generates completion scripts for bash, zsh, or fish shells."
    ~run:(fun ctx ->
      let shell = Cli.get_positionals ctx in
      let app = match !completion_app_ref with
        | Some a -> a
  ...
  cmd
  |> Cli.help_flag
  |> Cli.positionals "shell" ~placeholder:"SHELL" ~doc:"Shell to generate completion for (bash, zsh, fish)"

(* Subcommand: generate merlin *)
let cmd_generate_merlin =
  let cmd = Cli.command "merlin"
    ~doc:"Generate .merlin file for IDE support"
    ...
    ~run:(fun ctx ->
      ...
      List.iter (fun lib ->
        ...
        Buffer.add_string merlin_content (Printf.sprintf "B %s\n"
          (fp_to_string (current_dir </> fn "dist" </> fn "build" </> fn ("lib-" ^ Libname.to_string lib.Project.Library.name))))
      ...
      List.iter (fun exe ->
        ...
        Buffer.add_string merlin_content (Printf.sprintf "B %s\n"
          (fp_to_string (current_dir </> fn "dist" </> fn "build" </> fn exe.Project.Executable.name)))
      ) proj_file.Project.exes;

      (* Collect all package dependencies *)
      let all_deps = ref [] in
      List.iter (fun lib -> all_deps := lib.Project.Library.target.Target.target_obits.Target.target_builddeps @ !all_deps) proj_file.Project.libs;
      List.iter (fun exe -> all_deps := exe.Project.Executable.target.Target.target_obits.Target.target_builddeps @ !all_deps) proj_file.Project.exes;
      List.iter (fun test -> all_deps := test.Project.Test.target.Target.target_obits.Target.target_builddeps @ !all_deps) proj_file.Project.tests;

      let unique_deps = List.sort_uniq compare !all_deps in
      List.iter (fun (libname, _) ->
...
let cmd_generate_opam =
  let cmd = Cli.command "opam"
    ~doc:"Generate .opam file from project definition"
    ...
    ~run:(fun ctx ->
      ...
      Buffer.add_string opam_content (Printf.sprintf "version: \"%s\"\n" proj_file.Project.version);

      (* Synopsis and description *)
      if proj_file.Project.synopsis <> "" then
        Buffer.add_string opam_content (Printf.sprintf "synopsis: \"%s\"\n" proj_file.Project.synopsis);
      ...
      if proj_file.Project.authors <> [] then (
        ...
        List.iter (fun author ->
          Buffer.add_string opam_content (Printf.sprintf "  \"%s\"\n" author)
      ...
      if proj_file.Project.license <> "" then
        Buffer.add_string opam_content (Printf.sprintf "license: \"%s\"\n" proj_file.Project.license);

      (* Homepage *)
      if proj_file.Project.homepage <> "" then
        Buffer.add_string opam_content (Printf.sprintf "homepage: \"%s\"\n" proj_file.Project.homepage);
      ...
      List.iter (fun exe -> all_deps := exe.Project.Executable.target.Target.target_obits.Target.target_builddeps @ !all_deps) proj_file.Project.exes;
      List.iter (fun test -> all_deps := test.Project.Test.target.Target.target_obits.Target.target_builddeps @ !all_deps) proj_file.Project.tests;
      List.iter (fun bench -> all_deps := bench.Project.Bench.target.Target.target_obits.Target.target_builddeps @ !all_deps) proj_file.Project.benchs;
      List.iter (fun example -> all_deps := example.Project.Example.target.Target.target_obits.Target.target_builddeps @ !all_deps) proj_file.Project.examples;
...
let process_global_args ctx =
  ...
  if Cli.get_flag ctx "strict" then gconf.strict <- true;
...
let obuild_app =
  Cli.app "obuild"
    ...
    ~global_args:[
      ...
      (fun cmd -> Cli.flag "strict" ~doc:"Enable strict mode" cmd);
      (fun cmd -> Cli.option_string "findlib-conf" ~placeholder:"PATH"
          ~doc:"Path to findlib configuration" cmd);
      (fun cmd -> Cli.option_string "ocamlopt" ~placeholder:"PATH"
          ~doc:"Path to ocamlopt compiler" cmd);
      (fun cmd -> Cli.option_string "ocamldep" ~placeholder:"PATH"
          ~doc:"Path to ocamldep tool" cmd);
      (fun cmd -> Cli.option_string "ocamlc" ~placeholder:"PATH"
          ~doc:"Path to ocamlc compiler" cmd);
      (fun cmd -> Cli.option_string "cc" ~placeholder:"PATH"
          ~doc:"Path to C compiler" cmd);
      (fun cmd -> Cli.option_string "ar" ~placeholder:"PATH"
          ~doc:"Path to ar archiver" cmd);
      (fun cmd -> Cli.option_string "pkg-config" ~placeholder:"PATH"
          ~doc:"Path to pkg-config tool" cmd);
      (fun cmd -> Cli.option_string "ranlib" ~placeholder:"PATH"
          ~doc:"Path to ranlib tool" cmd);
    ]
...
let () =
  ...
  | Init.ProjectAlreadyExists ->
      eprintf "error: found another project file in this directory. cannot run init in an already existing project\n";
      exit 12
  | Init.AbortedByUser ->
